...
We evaluate \filopriori{} on an industrial dataset containing 100,502 test
executions across 4,552 builds (277 with failures). Results show that \filopriori{}
achieves a mean APFD of \textbf{0.7595}, significantly outperforming
state-of-the-art methods: NodeRank (0.6623, $+14.7\%$, $p < 0.001$) and
DeepOrder (0.6919, $+9.8\%$). Statistical analysis confirms the improvement
is significant with small effect size (Cliff's $\delta = 0.179$, Cohen's $d = 0.42$).
\filopriori{} achieves superior performance in 52.7\% of builds compared to
NodeRank, while NodeRank wins in only 31.0\%.
...
Our results show that \filopriori{} achieves a mean APFD of \textbf{0.7595},
significantly outperforming state-of-the-art methods: NodeRank by 14.7\%
($p < 0.001$) and DeepOrder by 9.8\%. The ablation study reveals that the Graph
Attention mechanism is the most critical component for modeling test relationships.
...
\begin{itemize}
    ...
    achieving APFD=0.7595, outperforming NodeRank by 14.7\% ($p < 0.001$) and
    DeepOrder by 9.8\%, with statistically significant improvements.
...
\toprule
\textbf{Method} & \textbf{Architecture} & \textbf{APFD} & \textbf{Std} & \textbf{p-value} & \textbf{Effect} \\
\midrule
\textbf{\filopriori{}} & \textbf{GAT + SBERT} & \textbf{0.7595} & \textbf{0.189} & -- & -- \\
DeepOrder~\cite{chen2023deeporder} & Deep NN & 0.6919 & 0.267 & $<$0.001*** & +9.8\% \\
NodeRank~\cite{li2024noderank} & Ensemble + Mutation & 0.6623 & 0.270 & $<$0.001*** & +14.7\% \\
...
\item Effect = improvement of \filopriori{} over baseline.
\end{tablenotes}
\end{table}
...
\begin{figure}[!t]
    \centering
    \includegraphics[width=0.9\columnwidth]{figures/fig_dual_stream_architecture.pdf}
...
\begin{figure}[!t]
    \centering
    \includegraphics[width=\columnwidth]{figures/fig_orphan_pipeline.pdf}
...
\begin{itemize}
    ...
    \item Empirical validation achieving \textbf{APFD=0.7595}, significantly
    outperforming NodeRank ($+14.7\%$, $p < 0.001$) and DeepOrder ($+9.8\%$).
    \item Cross-dataset validation on RTPTorrent achieving APFD=0.8376 across
    20 Java projects, outperforming 6 of 7 baselines.
...
($+17.0\%$), validating our hypothesis that modeling test relationships through
graphs is more effective than treating tests as independent entities.
