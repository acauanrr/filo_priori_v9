...
(N=277 builds). Filo-Priori (Hybrid) achieves a mean APFD of 0.6413, outperforming
all baselines including FailureRate (0.6289) by 2.0\%. Whiskers extend to 1.5 IQR.}
...
Filo-Priori achieves +14.6\% improvement over Random ($p < 0.001$) and +2.0\%
over the strongest baseline (FailureRate). Error bars represent 95\% bootstrap
...
\caption{Component contributions to Filo-Priori Hybrid performance. Graph Attention
(GATv2) provides the largest contribution (+17.0\%), followed by Structural
Stream (+5.3\%), Class Weighting (+4.6\%), and PhyloEncoder LITE (+0.5\%).
The hybrid architecture combines proven GATv2 with novel phylogenetic encoding.}
\label{fig:ablation}
\end{figure}
