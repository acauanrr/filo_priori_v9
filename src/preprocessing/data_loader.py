...
class DataLoader:
    ...
    def split_data(self, df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
        ...
        logger.info(f"🔒 Using GROUP-AWARE split (by Build_ID) to prevent leakage")
        ...
        logger.info(f"✅ Train set: {len(train_df)} samples ({train_df['Build_ID'].nunique()} builds)")
        logger.info(f"✅ Val set:   {len(val_df)} samples ({val_df['Build_ID'].nunique()} builds)")
        logger.info(f"✅ Test set:  {len(test_df)} samples ({test_df['Build_ID'].nunique()} builds)")
        ...
        if overlap_train_val or overlap_train_test or overlap_val_test:
            ...
            logger.info(f"✅ No build leakage: All splits are disjoint by Build_ID")
