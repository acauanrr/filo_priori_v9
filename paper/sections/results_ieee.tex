...
\begin{tabular}{lcccc}
\toprule
\textbf{Method} & \textbf{Mean APFD} & \textbf{95\% CI} & \textbf{p-value} & \textbf{vs Random} \\
\midrule
\textbf{\filopriori{}} & \textbf{0.6379} & [0.609, 0.669] & -- & \textbf{+14.0\%} \\
...
\begin{itemize}
    \item \filopriori{} achieves a mean APFD of \textbf{0.6379} (95\% CI: [0.609, 0.669]),
    representing the highest performance among all evaluated methods.
    \item This represents a \textbf{14.0\%} improvement over Random ordering
    ...
    by \textbf{+1.4\%}. While this improvement is not statistically significant
    ($p = 0.363$), it demonstrates that our approach can match and exceed
...
\filopriori{} achieves the highest APFD (0.6379) among all evaluated methods,
significantly outperforming random ordering by 14.0\% and surpassing the
strongest baseline (FailureRate) by 1.4\%.
\end{tcolorbox}
...
\begin{tabular}{llcc}
\toprule
\textbf{Parameter} & \textbf{Values Tested} & \textbf{Best} & \textbf{$\Delta$ Range} \\
\midrule
