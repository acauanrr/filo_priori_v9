...
\node[stagebox=stage1, right=1.2cm of $(orphan)!0.5!(ingraph)$] (stage1) {
    \textbf{Stage 1}\\
    KNN Similarity\\
    {\tiny (Euclidean, k=20)}
};
...
\node[stagebox=stage2, right=0.85cm of stage1] (stage2) {
    \textbf{Stage 2}\\
    Structural Blend\\
    {\tiny ($w_s = 0.35$)}
};
...
\node[stagebox=stage3, right=0.85cm of stage2] (stage3) {
    \textbf{Stage 3}\\
    Temperature\\Softmax\\
    {\tiny ($T = 0.7$)}
};
...
\node[stagebox=stage4, right=0.85cm of stage3] (stage4) {
    \textbf{Stage 4}\\
    Alpha Blending\\
    {\tiny ($\alpha = 0.55$)}
};
...
% === OUTPUT ===
\node[outputbox, right=1.3cm of stage4] (output) {Orphan\\Scores};

% === ARROWS ===
\draw[arrow] (orphan.east) -- ++(0.35,0) |- (stage1.west);
\draw[arrow] (ingraph.east) -- ++(0.35,0) |- (stage1.west);
...
\node[font=\scriptsize\bfseries] at (10.9, -2.3) {After};
\node[font=\tiny, align=center] at (10.9, -2.9) {
    Differentiated:\\
    {[0.29, 0.51]}
};
