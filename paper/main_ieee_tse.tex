...
presents both architectural differences and empirical performance comparison on
our industrial dataset:
...
\begin{tabular}{lcccc}
\toprule
\textbf{Approach} & \textbf{Architecture} & \textbf{GNN} & \textbf{APFD} & \textbf{$\Delta$ vs Random} \\
\midrule
DeepOrder~\cite{chen2023deeporder} & Deep NN & No & 0.650 & +22.0\% \\
\textbf{\filopriori{}} & \textbf{GAT + SBERT} & \textbf{Yes} & \textbf{0.641} & \textbf{+20.3\%} \\
FailureRate & Heuristic & No & 0.555 & +4.1\% \\
RETECS~\cite{spieker2017reinforcement} & RL (Q-learning) & No & 0.458 & -14.0\% \\
\bottomrule
\end{tabular}
\begin{tablenotes}
\small
\item Results on industrial dataset (371 builds with failures). DeepOrder and \filopriori{}
show no statistically significant difference ($p = 0.43$, Wilcoxon signed-rank test).
\end{tablenotes}
\end{table}

Key differentiators:
\begin{itemize}
    \item \textbf{Graph-based modeling}: \filopriori{} is the only approach that
    explicitly models test relationships using Graph Attention Networks.
    \item \textbf{Dual-stream architecture}: We combine semantic (SBERT) and
    structural (GAT) features, unlike DeepOrder which uses only historical features.
    \item \textbf{Competitive performance}: While DeepOrder achieves slightly
    higher APFD (0.650 vs 0.641), the difference is not statistically significant,
    and \filopriori{}'s graph-based design provides interpretability advantages.
\end{itemize}
