...
% FFN Block detail
\node[font=\tiny, right=0.1cm of sem_ffn1, align=left, text=gray] {
...
% === STRUCTURAL STREAM (Right) ===
\node[inputbox] (str_in) at (5, 0) {Features\\(19-d)};
\node[inputbox, right=0.3cm of str_in] (graph_in) {Graph\\$\mathcal{G}$};

\node[layerbox=structuralcolor, below=0.4cm of $(str_in)!0.5!(graph_in)$] (str_proj) {Linear 19$\rightarrow$256};
\node[layerbox=structuralcolor, below=0.3cm of str_proj] (gat1) {GAT Layer 1 (4 heads)};
\node[layerbox=structuralcolor, below=0.3cm of gat1] (gat2) {GAT Layer 2 (1 head)};
\node[layerbox=structuralcolor, below=0.3cm of gat2] (str_ln) {LayerNorm};

% GAT detail
\node[font=\tiny, right=0.1cm of gat1, align=left, text=gray] {
...
\node[layerbox=fusioncolor, below=1.2cm of $(sem_out)!0.5!(str_out)$, minimum width=4cm] (cross1) {
    Cross-Attention: Semantic $\rightarrow$ Structural
};
\node[layerbox=fusioncolor, below=0.3cm of cross1, minimum width=4cm] (cross2) {
    Cross-Attention: Structural $\rightarrow$ Semantic
};
\node[layerbox=fusioncolor, below=0.3cm of cross2, minimum width=3cm] (concat) {Concatenate [512-d]};

% Fusion label
\node[font=\small\bfseries, fusioncolor, left=0.5cm of cross1] {Fusion};
...
% Structural stream
\draw[arrow] (str_in) -- ++(0,-0.2) -| (str_proj);
\draw[arrow] (graph_in) -- ++(0,-0.2) -| (str_proj);
